...
// OTHER SECTION ENDS
        ...
        public function withdrawdatatables()
        {
             ...
             return Datatables::of($datas)
                                ...
                                ->addColumn('action', function(Withdraw $data) {
                                    ...
                                    $action .= '<a data-href="' . route('admin-withdraw-accept',$data->id) . '" data-toggle="modal" data-target="#confirm-delete"> <i class="fas fa-check"></i> Accept</a><a data-href="' . route('admin-withdraw-balanceReject',$data->id) . '" data-toggle="modal" data-target="#confirm-delete1"> <i class="fas fa-trash-alt"></i>Reject</a>';
                                    }
                                    $action .= '</div>';
                                    return $action;
                                }) 
        ...
        public function balanceReject($id)
        {
            $withdraw = Withdraw::findOrFail($id);
            $account = User::findOrFail($withdraw->user->id);
