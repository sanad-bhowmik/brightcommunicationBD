			...
			{{ csrf_field() }}
					...
					<div class="checkout-area">
						<div class="tab-content" id="pills-tabContent">
												...
												<div class="row">
													<div class="col-lg-12">
														...
														@if($gs->is_instamojo == 1)
															...
															@if($gs->is_mercado == 1)
															<a class="nav-link payment" data-val="mercadopago" data-show="yes" data-form="{{route('mercadopago.submit')}}" data-href="{{ route('front.load.payment',[ 'mercadopago', 0]) }}" id="v-pills-tab10-tab" data-toggle="pill" href="#v-pills-tab10" role="tab" aria-controls="v-pills-tab10" aria-selected="false">
																<div class="icon">
																	<span class="radio"></span>
																</div>
																<p>
							...
							<input type="hidden" name="referral_user_id" id="referral_user_id" value="">
							<input type="hidden" name="referral_discount" id="referral_discount" value="">
				...
				<div class="col-lg-4">
		            ...
		            <div class="total-price">
		              ...
		              <p>
						...
						<div id="coupon-box" class="cupon-box">
						...
						<div id="referral-box" class="cupon-box">
						    ...
						    <form id="check-rcode-form" class="coupon">
						        <input type="text" placeholder="Enter Referral code" id="rcode" required="" autocomplete="off">
						        <button id="rcode-submit" type="submit">{{ $langg->lang134 }}</button>
						    </form>
...
@if($val = Session::has('already') ? Session::get('already') : null)

$('#referral-box').addClass('d-none');
...
$("#wallet-form").on('submit', function (e) {
	e.preventDefault();
        var val = 0;
		val = parseFloat($("#wallet-amount").val());
            ...
            $.ajax({
                    ...
                    success:function(data){
                        if(data == 0){
							...
							console.log(data);
							console.log(val);
							//data[1] = parseFloat(data[1])+parseFloat(val);
							$("#wallet-amount").val('');
							$('#wallet').click();
...
// referral code check
	...
	$("#check-rcode-form").on('submit', function (e) {

			e.preventDefault();
			var val= $("#rcode").val();
			var uid = {{Auth::guard('web')->user()->id}};

       		 var total = $("#ttotal").val();
        	 var ship = 0;
            $.ajax({
                    type: "GET",
                    url:mainurl+"/carts/referralcode/check",
					data:{code:val, total:total, shipping_cost:ship,uid:uid},
                    success:function(data){

                        if(data == 0)
                        {
                        	toastr.error("Referral Code Not Found");
                           // $("#code").val("");
                        }
                        else
                        {
							//console.log(data[2]);
                           // $("#check-rcode-form").toggle();
						   $(".discount-bar").removeClass('d-none');
						   $('#referral_code').val(data[1]);
						   $('#referral_user_id').val(data[0]);
						   $('#referral_discount').val(data[3]);
						  // console.log(data);
                            $("#rcode-submit").addClass('d-none');
							$("#rcode").prop('readonly',true);

							//referal discount
							$('#total-cost').html(data[3]+'{{ $curr->sign }}');
							$('#discount').html(data[3]+'{{ $curr->sign }}');
							$('#grandtotal').val(data[2]);
							$('#tgrandtotal').val(data[2]);
      ...
      if(ttotal % 1 != 0)
      {
		...
		if(pos == 0){
			...
			$('#final-cost').html(ttotal+'{{ $curr->sign }}')
                        	...
                        	toastr.success("Referral Code Applied");
